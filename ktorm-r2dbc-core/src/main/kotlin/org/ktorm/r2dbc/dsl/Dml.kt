 ...
 */
public suspend fun <T : BaseTable<*>> Database.update(table: T, block: UpdateStatementBuilder.(T) -> Unit): Int {
 ...
 */
public suspend fun <T : BaseTable<*>> Database.batchUpdate(
    table: T,
 ...
 * @return the effected row count.
 */
public suspend fun <T : BaseTable<*>> Database.insert(table: T, block: AssignmentsBuilder.(T) -> Unit): Int {
 ...
 * row counts for each expression.
 *
 * Note that this function is implemented based on [Statement.add] and [Statement.executeBatch],
 * and any item in a batch operation must have the same structure, otherwise an exception will be thrown.
 *
 * Usage:
 *
 ...
 * @return the effected row counts for each sub-operation.
 */
public suspend fun <T : BaseTable<*>> Database.batchInsert(
    table: T,
 ...
 */
public suspend fun Query.insertTo(table: BaseTable<*>, vararg columns: Column<*>): Int {
 ...
 * @since 2.7
 */
public suspend fun <T : BaseTable<*>> Database.delete(table: T, predicate: (T) -> ColumnDeclaring<Boolean>): Int {
 ...
 * @since 2.7
 */
public suspend fun Database.deleteAll(table: BaseTable<*>): Int {
...
public open class AssignmentsBuilder {
    ...
    private fun Column<Any>.checkAssignableFrom(value: Any?) {
        ...
        val proxy = Proxy.newProxyInstance(javaClass.classLoader, arrayOf(Statement::class.java), handler)

        try {
            sqlType.bindParameter(proxy as Statement, 1, value)
